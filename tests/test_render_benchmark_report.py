...
def _write_quant(base: Path, quant: str, size_bytes: int, warm: float, cosines, pt_warm: float):
    d = base / f"benchmark-arm64-{quant}"
    d.mkdir(parents=True)
    rows = [
        _row(implementation="ltembed", mode="warm_latency", scenario="single/medium", mean_ms=f"{warm:.6f}"),
        _row(implementation="pytorch", mode="warm_latency", scenario="single/medium", mean_ms=f"{pt_warm:.6f}"),
    ]
...
class RenderBenchmarkReportTests(unittest.TestCase):
    def test_recommends_smallest_quant_passing_quality_gate(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            base.mkdir()
            _write_quant(base, "IQ4_XS", 190_000_000, warm=8.0, cosines=[0.95, 0.96], pt_warm=42.0)
            _write_quant(base, "Q5_K_M", 230_000_000, warm=9.5, cosines=[0.99, 0.988], pt_warm=42.0)
            _write_quant(base, "Q8_0", 350_000_000, warm=12.0, cosines=[0.997, 0.996], pt_warm=42.0)

            results = mod.collect_results(base)
            self.assertEqual([r["quant"] for r in results], ["IQ4_XS", "Q5_K_M", "Q8_0"])

            recommended, _reason = mod.recommend(results)
            # IQ4_XS fails the gate (0.95); Q5_K_M is the smallest that passes.
            ...
            self.assertAlmostEqual(q5["speedup_vs_pytorch"], 42.0 / 9.5)
    ...
    def test_recommend_falls_back_to_highest_mean_when_none_pass_gate(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir) / "bench-results"
            base.mkdir()
            _write_quant(base, "IQ4_XS", 190_000_000, warm=8.0, cosines=[0.90, 0.91], pt_warm=42.0)
            _write_quant(base, "Q5_K_M", 230_000_000, warm=9.5, cosines=[0.95, 0.94], pt_warm=42.0)
            results = mod.collect_results(base)
